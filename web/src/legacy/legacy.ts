// Legacy page renderers: the pages that have not moved to Vue components yet.
// They are the original imperative implementations, kept behaviour-compatible so
// the browser regression suites keep passing while the shell runs on Vue. Each
// page is deleted from this file as it is migrated.
//
// The file is a verbatim port of the pre-Vue implementation, so it is exempt
// from type checking until its last page moves: the browser suites, not the
// compiler, are what verify its behaviour during the transition.
// @ts-nocheck
/* eslint-disable @typescript-eslint/no-explicit-any */
import { api, pageSignal } from '../core/api'
import { openModal } from '../core/dialog'
import {
  busy, button, cell, el, empty, emptyState, field, icon, iconButton, loading,
  notice, pageHeader, pagination, renderError, table,
} from '../core/dom'
import { compactNum, date, num, positiveInt, utcDay } from '../core/format'
...
async function renderDashboard(view) {
  ...
  const refresh = iconButton("刷新仪表盘", "refresh-cw", () => reloadPage());
...
function renderTrendChart(data) {
  ...
  for (let i = 0; i <= 4; i++) {
    ...
    svg.append(svgEl("line", { x1: xPercent(left), x2: xPercent(width - 20), y1: y, y2: y, class: "chart-grid" }),
      svgEl("text", { x: xPercent(left - 12), y: y + 4, "text-anchor": "end", class: "chart-label" }, compactNum(tick * i)));
  }
...
async function renderBuiltin(view) {
  ...
  testForm.addEventListener("submit", (event) => {
    ...
    busy(testButton, "正在测试…", async () => {
      ...
      try {
        const result = await api("/api/builtin-rules/test", { method: "POST", body: { text: testText.value }, signal: pageSignal() });
  ...
  empty(view).append(pageHeader("内置广告库", "全局防护 · 对所有群组生效",
    [iconButton("刷新内置广告库", "refresh-cw", () => reloadPage())]), ruleTabs(), errors, version, progress, region);
...
async function renderRules(view) {
  ...
  let rules = [];
  const refresh = iconButton("刷新规则", "refresh-cw", () => reloadPage());
  ...
  try {
    ...
    accountForm.addEventListener("submit", (event) => {
      ...
      busy(saveUser, "保存中…", async () => {
        ...
        try {
          ...
          state.authenticated = false; toast("用户名已更新，请重新登录。");
    ...
    passwordForm.addEventListener("submit", (event) => {
      ...
      busy(savePassword, "保存中…", async () => {
        try {
          ...
          state.authenticated = false; toast("密码已更新，请重新登录。");
    ...
    empty(view).append(pageHeader("设置", "当前登录：" + account.username),
      el("div", { class: "settings-sections" },
        ...
        runtimeSection));
  } catch (err) { renderError(view, err, () => reloadPage()); }
}


export type LegacyRoute = 'dashboard' | 'rules' | 'builtin' | 'audit' | 'settings'
export type LegacyRenderer = (view: HTMLElement) => Promise<void> | void

// The router maps each not-yet-migrated route here.
export const legacyPages: Record<LegacyRoute, LegacyRenderer> = {
